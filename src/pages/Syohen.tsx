    ...
    <div className="page-main-container">
      <header className="header-underline-bold border-(--gs-accent)">
        <div className="flex flex-col md:flex-row justify-between items-end">
          ...
          <div className="flex p-1 rounded-2xl border bg-slate-100/50 border-slate-200">
            {years.map(year => (
              <button 
                key={year} 
                ...
                className={`px-6 py-2 rounded-xl text-[11px] font-black ${
                  activeYear === year 
                    ? "bg-(--gs-accent) text-white shadow-lg" 
      ...
      <div key={activeYear} className="grid grid-cols-1 md:grid-cols-2 gap-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {currentActivities.map((activity: any) => {
          const scheme = COLOR_SCHEMES[activity.category] || COLOR_SCHEMES["事務改善"];
          return (
            <div 
              key={activity.id} 
              ...
              {activity.pdfUrl && (
                <div className="px-10 pb-10">
                  <button 
                    ...
                    className="w-full py-5 rounded-2xl flex items-center justify-center gap-4 text-[11px] font-black uppercase tracking-[0.3em] group/btn bg-(--gs-accent) text-white hover:bg-(--gs-accent)/80 shadow-lg shadow-black/10"
                  >
